...
app.use(new CSRF({invalidSessionSecretMessage: 'Invalid session secret', invalidSessionSecretStatusCode: 403, invalidTokenMessage: 'Invalid CSRF token', invalidTokenStatusCode: 403}))
...
router.get('/map/place/textsearch/:input', async(ctx, next) => {
    ...
    try {
        let opts = {
            uri: `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURI(_.trim(ctx.params.input))}&types=establishment&language=zh-CN&components=country:us&key=${G_API_KEY}`,
            json: true
        }
    ...
    try {
        let opts = {
            uri: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${ctx.params.maxWidth}&maxheight=${ctx.params.maxHeight}&photoreference=${ctx.params.ref}&key=${G_API_KEY}`,
            encoding: null
        }
