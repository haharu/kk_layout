...
self.addEventListener('install', (event) => {
    ...
    event.waitUntil(global.caches.open(CACHE_NAME).then((cache) => {
...
self.addEventListener('fetch', (event) => {
    ...
    if (requestUrl.pathname.slice(0, 4) == '/api') {
        if (DEBUG) {
            console.log(`[SW] Ignore api ${requestUrl.pathname}`);
        }
        return;
    }
    ...
    if (requestUrl.pathname == '/version.txt') {
        if (DEBUG) {
            console.log(`[SW] Ignore version.txt`)
        }
        return;
    }
